...
| Revision | **r2 (Engineering Consolidation Revision)** — supersedes r1 in place; one canonical specification only |
...
| 14 | Validity program expanded (content, construct, internal, external, predictive, **reproducibility**, **security validity**); criterion coefficients reframed as pre-registered targets ([§64](#64-the-six-fold-validity-program)) | Correction |
...
- **[PART X — EMPIRICAL MEASUREMENT SCIENCE & PSYCHOMETRIC CALIBRATION](#part-x--empirical-measurement-science--psychometric-calibration)**
  ...
  - [64. THE SIX-FOLD VALIDITY PROGRAM](#64-the-six-fold-validity-program)
...
| $\mathcal{B}$ | Authorization boundaries, role-based access controls, out-of-band security gates | Permission model and token scope policy |
...
flowchart TD
    ...
    H -->|pass| I["9 UPDATE — prune stale context, record validated progress"]
    H -->|fail| J["9 RECOVER — isolate failure, restore snapshot, re-plan"]
    J --> F
    I --> K["10 DELIVER — emit verifiable deliverable or decision brief"]
    K --> L["11 STOP / ESCALATE — halt when complete; escalate when authority or data is missing"]
```
...
| ACT | Dispatch bounded tool call, code execution, or API request | F08, F05 |
| VERIFY | Mechanically test state delta against acceptance criteria | F07 |
| RECOVER | Catch exceptions, isolate failure modes, backtrack to snapshot | F06, F08 |
...
Each factor is an empirical efficiency metric normalized on the unit interval $[0.0, 1.0]$:
...
| **PROFILE S-SEC** | High-Assurance Vault (Official Level 4/5) | Hardware microVM isolation (gVisor `runsc` / Firecracker); physical TPM2 / AMD SEV-SNP enclave; kernel eBPF egress drop |
...
STOCHASTIC_PARAMETERS         # Distribution family + parameters for E_{t+1} ~ P(·) (§23)
...
| D08 | Planning, Search & Optimization | Operations Research | Dynamic routing, scheduling, MILP, critical paths, heuristics |
| D09 | Decision-Making Under High Uncertainty | Decision Sciences | Bayesian inference, CVaR, Monte Carlo, value-of-information |
| D10 | Memory, Context & State Integrity | Cognitive Architecture | Long-horizon retention (50+ turns), ACE playbooks, timestamp hygiene |
...
| D12 | Multimodal Perception & Grounding | Perceptual Sciences | Spatial SVG graphs, spectrograms, thermal grids, DICOM imaging |
...
|---|---|---|---|---|
| B01 | Epistemic Integrity & Truth Calibration | 7.00% | 7.00% | Formal logic parsers, AST claim diffs |
| B02 | Reasoning, Mathematics, Logic & Formal Methods | 9.00% | 16.00% | Z3 theorem prover, SymPy, Coq/Lean |
| B03 | Research, Retrieval, Browsing & Evidence Synthesis | 6.00% | 22.00% | Provenance DAGs, DOI/Git cross-check |
| B04 | Multimodal Perception & Grounded Spatial Generation | 6.00% | 28.00% | IoU geometry checks, coordinate masks |
| B05 | Coding, Systems Engineering & Fault Localisation | 8.00% | 36.00% | Clang ASan/TSan, cargo test, pytest |
| B06 | Tool Use, API Integration & OS Computer Interaction | 7.00% | 43.00% | Mock HTTP engines, JSON-RPC assertion |
| B07 | Planning, Search, Optimization & Decision Analysis | 7.00% | 50.00% | OR-Tools, MILP solvers, SimPy models |
| B08 | Context, Working Memory, State & Long-Horizon Hygiene | 6.00% | 56.00% | Schema linters, lexical token scans |
| B09 | Learning, Adaptation, Generalization & Error Reversal | 6.00% | 62.00% | Automated mutation test suites |
| B10 | Human Pedagogy, Social Reasoning & Negotiation | 5.00% | 67.00% | Constraint matrices, pragmatic models |
| B11 | Constrained Creativity, Architecture & System Design | 4.00% | 71.00% | CoolProp thermo, physics constraint |
| B12 | Scientific Reasoning, Experiment Design & Deep R&D | 5.00% | 76.00% | Factorial power tests, causal DAGs |
| B13 | Multi-Agent Coordination, Consensus & Byzantine Safety | 4.00% | 80.00% | Distributed quorum state checks |
| B14 | Safety, Zero-Trust Security, Authorization & Injection | 7.00% | 87.00% | Seccomp audits, token regex monitors |
| B15 | Strategic Monetization & Unit Economic Value Conversion | 4.00% | 91.00% | Mathematical financial cohort solvers |
...
The weights in §9.1 are **design weights** ($W_{\text{design}}$): a pre-registered, expert-justified allocation chosen before observing any evaluation data. They are **not** claimed to be statistically optimal. AIST distinguishes:

- $W_{\text{design}}$ — the canonical weights above; used for all official scoring until superseded.
- $W_{\text{empirical}}$ — any future weight profile derived from calibration data (e.g., via the TDI program of [§61](#61-human-expert-baselines--multi-factor-task-difficulty-index-tdi), IRT information weighting of [§63](#63-item-response-theory-irt-4pl-latent-trait-modeling), or criterion-validity regression of [§64](#64-the-six-fold-validity-program)). An empirical profile may replace $W_{\text{design}}$ **only** after: (a) full statistical calibration release, (b) documented non-inferiority on the validity program, and (c) a breaking-change entry under [§73.3](#733-backward-compatibility--migration-policy).
...
| **LEVEL 2** | High / Systemic Financial Risk | Independent PRM verification; assumption falsification check | Mandatory pre-action state snapshot (Git tree / WAL LSN / DB transaction) | cgroups quotas; snapshot verified before mutation |
...
Validator selection is declared per item in the task card's `VALIDATOR_STACK`; campaigns must report the distribution of validator tiers used per battery so that capability claims can be matched to evidentiary strength ([§50](#50-process-reward-model-prm-dense-step-annotation-pipeline) audit trail).
...
|---|---|---|---|
| F1 | Malformed Metadata / Schema | 20.0% | Corrupts Task Card YAML/JSON types; passes invalid enum values, null constraints, or broken IDs |
| F2 | Corrupted Expected State | 20.0% | Seeds false ground-truth answers or impossible mathematical target values into private answer keys |
| F3 | Validator Defect / Flawed Oracle | 20.0% | Inverts unit test assertions (`assert False`); injects syntax errors or infinite loops into test suites |
| F4 | Environment / Tool Sandbox Failure | 20.0% | Simulates dead mock HTTP servers; unlinks local shell binaries; remounts /tmp as read-only unexpectedly |
| F5 | Adversarial Benchmark Trap | 20.0% | Injects adversarial canary payloads into evaluator logging streams to test scorer injection resistance |
...
|---|---|---|
| **Repeated-run variance** | Per [§62.1](#621-minimum-repetition--execution-policy): N ≥ 5 seeds (stochastic modes), N ≥ 10 (Level 2/3 certification) | Per-item SD; ICC(2,k) across seeds |
...
2. **A higher-order general capability score ($g$) may be estimated only when statistically justified:** the second-order factor model must show acceptable fit (e.g., CFI/TLI ≥ 0.95, RMSEA ≤ 0.06), non-degenerate loadings, and adequate sample ($N_{\text{systems}} \ge 30$ or a pre-registered Bayesian prior). The loadings and fit indices are published with the estimate.
...
## 64. THE SIX-FOLD VALIDITY PROGRAM
...
VERSION_MANIFEST: Dict[str, str] = {
    "aist_standard_version": "AIST-2026.09-r2",
...
class AISTMasterRunner:
    ...
    def export_all_artifacts(self) -> None:
        ...
        with scorecard_path.open("w", encoding="utf-8") as f:
            ...
            f.write(f"AIST_RAW / AIST_CAP:       {card['AIST_RAW']:.2f} / 100.00\n")
...
| LEVEL 5 | Master Autonomous System (AGI Operational Prototype) | 96.00% ($\text{AIST}_{\text{OP}}$) | Zero Hard Gate violations; Core Score ≥ 95.00% & B16 Score ≥ 95.00%; Cascade Robustness Retention ≥ 0.92; automated PRM/DPO data export verified; Reproducibility Tier R3 (full audit); effective task horizon $H_{50}$ ≥ 8.0 hours |
...
|---|---|---|
| Test-Time Compute & Generative Process Reward Models (PRMs) | Zhao et al. (2025/2026); ThinkPRM Consortium (2026); VersaPRM / ToolPRMBench | Powers [Section 50](#50-process-reward-model-prm-dense-step-annotation-pipeline) (Mechanically Grounded Relative Progress PRM Step Supervision, $r_t \in \{-1, 0, +1\}$) |
| Self-Evolving Trajectory RL & Multi-Turn Policy Optimization | StarPO / RAGEN (2025/2026); Systematic Survey on Self-Evolving Agents (2026) | Powers [Part VII](#part-vii--the-hermetic-self-evolution-engine--data-flywheel) (Autonomous Data Flywheel, DPO Preference Mining via Recursive Tournament Voting) |
| Long-Horizon Context Collapse & Policy Degradation | Panavas et al. (2026), arXiv:2607.25398 (HANDBOOK.md) | Governs Battery B08 & Domain D10 (50+ turn dynamic playbook pruning, ISO-8601 timestamp invalidation) |
| Instruction Stacking Collapse & Prompt Bloat Degradation | Anand & Chattaraj (2026), arXiv:2608.02639 | Governs [Section 7](#7-measured-vs-rejected-behavior-anti-gaming--stopping-intelligence) & Part I (Anti-Bloat Mandate, Instruction Economy, Zero-Fluff Structural Rules) |
| Multi-Turn Adaptive Red-Teaming & Adversarial Agents | Jain et al. (2026), arXiv:2607.18063 (Adaptive Adversaries) | Governs Battery B14 & [Section 54](#54-the-twelve-structural-perturbation-families-p_01-dots-p_12) (Adaptive Multi-Turn Injection, Cascading Fault Injections) |
| Out-of-Band Physical Defenses Against Agent Hijacking | Narisetty et al. (2026), arXiv:2606.26479 | Enforces Hard Gate G3 & [Section 12](#12-consequence-risk--enforcement-model) (HRR out-of-band hardware cryptotoken verification for Level 2/3 actions) |
| Long-Horizon CLI & Repository Autonomous Repair | Frontier Agent Evaluation Consortium (Sep 2026), Terminal-Bench 2.1 / DeepSWE | Grounds Batteries B05, B06, B16 (Linux container sandboxing, POSIX execution diffs, Cargo/Miri tests) |
| Task Duration Scaling & Human Completion Time Horizons | METR Research (2024–2026), Time Horizons & RE-Bench | Grounds [Section 61](#61-human-expert-baselines--multi-factor-task-difficulty-index-tdi) (Human baseline protocols, TDI index, effective completion horizons $H_{50}$ / $H_{80}$) |
...
```text
AIST Standard Version              AIST-2026.09-r2
