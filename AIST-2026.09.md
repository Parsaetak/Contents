...
Each $E_x \in [0, 1]$: $E_{\text{tool}}$ is exactly $0$ when failed and redundant calls exhaust the call budget (the reference core floors it at zero, never negative); values above baseline saturate at 1 so that efficiency never subsidizes poor utility. Baselines are versioned; a baseline change is a scoring-semantics change under [§73.3](#733-backward-compatibility--migration-policy).
...
5. **Absence of benchmark leakage:** contamination canaries, exposure monitoring ([§57.3](#573-exposure-monitoring--dynamic-invalidation-protocol)), and the matched contamination delta ($\Delta_{\text{matched}}$, [§64.2](#642-contamination-adjusted-scoring-metrics)) show no holdout entanglement.
...
All training data harvested during Zone 1 / Zone 2 evaluation runs is exported by the reference runner as an immutable, schema-validated JSONL file (`<run_id>_training_manifest.jsonl`, [§68](#68-automated-csv-master-scorecard-and-training-manifest-exporter)). Every line is exactly one JSON object: the first line is the manifest header, and every subsequent line is one harvested-record envelope.

```text
Line 1     MANIFEST_HEADER    {"type": "MANIFEST_HEADER", "$schema": "https://aist-standard.org/schemas/2026.09/training_manifest.json", "manifest_version": "AIST-2026.09-HARVEST", "aist_standard_version": <version>, "run_id": <id>, "timestamp_utc": <iso8601>, "harvesting_zone": "ZONE_1_PUBLIC_DEV" | "ZONE_2_SYNTHETIC_POOL", "evaluated_model_id": <id>, "scaffold_identity": <id>, "record_counts": {"PRM_STEP": n1, "DPO_PAIR": n2, "STAR_TRACE": n3}}
Line 2..N  record envelope    {"type": "PRM_STEP" | "DPO_PAIR" | "STAR_TRACE", "run_id": <id>, "data": { ...payload... }}
```

**Structural validation rules** (machine-checkable; enforced by the reference runner at export and required of any conforming consumer at ingestion):

1. Every line parses as a single JSON object; blank lines and trailing non-JSON content fail validation.
2. Line 1 carries `"type": "MANIFEST_HEADER"`; no other line may carry that type.
3. Every subsequent line's `type` is exactly one of `PRM_STEP`, `DPO_PAIR`, `STAR_TRACE`.
4. Every record envelope's `run_id` equals the header's `run_id`.
5. The header's `harvesting_zone` is one of `ZONE_1_PUBLIC_DEV`, `ZONE_2_SYNTHETIC_POOL` — a manifest may never originate from Zone 3/4 ([§49.1](#491-the-invariant-isolation-rules-of-the-flywheel)); the reference runner's manifest writer is physically disabled there.
6. **Count parity:** the number of lines of each record type equals the header's declared `record_counts`.
7. **Payload contracts:** `PRM_STEP` data carries `step_reward ∈ {-1.0, 0.0, +1.0}` and a `step_label` from the RPE table of [§50](#50-process-reward-model-prm-dense-step-annotation-pipeline); `DPO_PAIR` data carries `chosen_completion`, `rejected_completion`, `divergence_step_index`, and `simpo_margin` ([§51.1](#511-contrastive-mining-criteria)); `STAR_TRACE` data carries the five-stage repair tuple of [§52](#52-self-correction--backtracking-trajectory-mining-star).
8. **Immutability:** manifests are write-once artifacts; any post-export mutation is detected by line-count and hash re-verification and fails ingestion.

<details>
<summary>Example manifest (JSONL; format illustration, not empirical data)</summary>


```json
{"type": "MANIFEST_HEADER", "$schema": "https://aist-standard.org/schemas/2026.09/training_manifest.json", "manifest_version": "AIST-2026.09-HARVEST", "aist_standard_version": "AIST-2026.09", "run_id": "aist-run-20260906-8812a-f3", "timestamp_utc": "2026-09-06T19:20:00Z", "harvesting_zone": "ZONE_2_SYNTHETIC_POOL", "evaluated_model_id": "frontier-reasoner-70b", "scaffold_identity": "REP-v2026.09", "record_counts": {"PRM_STEP": 1, "DPO_PAIR": 1, "STAR_TRACE": 1}}
{"type": "PRM_STEP", "run_id": "aist-run-20260906-8812a-f3", "data": {"record_id": "PRM-D05-01-STEP-04", "task_id": "D05-01", "battery_id": "B05_SoftwareEngineering", "record_type": "PRM_STEP_SUPERVISION", "domain": "Software & Systems Engineering", "difficulty_tier": "L3", "context_prefix": "User requested lock-free queue in C++20. Step 1 defined Node struct. Step 2 allocated atomic head/tail. Step 3 implemented push().", "step_content": "Implement pop() using tagged pointer to resolve ABA: uintptr_t old_head = head.load(); ...", "step_reward": 1.0, "verification_evidence": {"validator_type": "TIER_4_UNIT_TEST_TSAN", "command_executed": "clang++ -fsanitize=thread -O3 test_queue.cpp -o test && ./test", "exit_code": 0, "stdout_summary": "100000 iterations passed with 0 data races"}}}
{"type": "DPO_PAIR", "run_id": "aist-run-20260906-8812a-f3", "data": {"record_id": "DPO-B02-02-PAIR-01", "task_id": "B02-02", "battery_id": "B02_ReasoningFormal", "record_type": "DPO_PREFERENCE_PAIR", "prompt": "Synthesize a satisfiable SMT-LIB (Z3) formulation computing a 4D trajectory for 10 aircraft entering terminal airspace...", "chosen_completion": "(set-logic QF_LRA)\n(declare-fun t_alt (Int) Real) ... [Verified Sat by Z3]", "rejected_completion": "(set-logic QF_LRA)\n(declare-fun t_alt (Int) Real) ... [Violates 3 NM separation at t=120s]", "divergence_step_index": 14, "critique": "Rejected completion formulated separation constraint as a disjunction that allowed concurrent boundary collapse under z3 branch optimization.", "simpo_margin": 0.42}}
{"type": "STAR_TRACE", "run_id": "aist-run-20260906-8812a-f3", "data": {"record_id": "STAR-C02-RECOVERY-02", "task_id": "C02", "battery_id": "B05_SoftwareEngineering", "record_type": "STAR_SELF_CORRECTION_TRACE", "initial_fault": {"command": "pg_waldump -p /var/lib/postgresql/data/pg_wal 000000010000001A0000008F", "error_payload": "FATAL: error in WAL record at 1A/8F042C80: invalid record length"}, "diagnostic_reflection": "Direct WAL replay fails at sector boundary 1A/8F042C80 due to an uncompleted transaction write during the hard power crash. Force-starting without recovery would cause catalog inconsistency.", "rollback_action": "restore database volume from pre-action CoW snapshot; copy pg_wal to cold archive", "remediation_action": "PITR: create recovery.signal, set restore_command and recovery_target_lsn = 1A/8F042C80 (last valid record), then promote after checksum verification", "verification_result": "Recovery reached target LSN; pg_is_in_recovery() = true during replay and false after clean promotion; 0 corrupted pages; 0 missing committed transactions."}}
```
...
1. **Anomaly Detection:** If a model exhibits near-zero latency ($< 10\%$ baseline), zero tool search iterations, and exact string identity against private answer keys, the run is flagged as `SUSPECT_CONTAMINATED` — a *contamination signal* under the classification of [§64.2](#642-contamination-adjusted-scoring-metrics).
2. **Canary Emission Check:** If an evaluated system emits a registered canary string in unprompted generation, the affected battery's scores are **quarantined immediately — excluded from publication, never zeroed** — and the run is flagged as carrying *contamination evidence* ([§64.2](#642-contamination-adjusted-scoring-metrics)). The deterministic investigation pipeline (quarantine → reproduce → trace → classify) then determines whether contamination is confirmed; emission alone initiates the pipeline and is never treated as confirmed contamination by itself.
...
**Sample-size honesty.** The per-task cohort of $N = 5$ specialists is a *feasibility minimum* for producing a per-task time anchor and success rate; its uncertainty is wide, and it does not by itself identify the TDI regression of [§61.2](#612-the-empirical-task-difficulty-index-tdi). The GLMM coefficients $\boldsymbol{\beta}$ are estimated from the pooled cross-domain calibration sample at a calibration release (maturity M2+); the minimum scientifically defensible pooled sample for that estimation — and the recommended calibration sample above it — are declared in the statistical calibration manifest when the release exists ([§49.2](#492-canonical-task-distribution--calibration-sets)). Per-task anchors from a 5-specialist cohort are never presented as precise population quantities; they are reported with their empirical dispersion, and computational feasibility (running the model) is never conflated with statistical identification (having enough independent observations to constrain it).
...
**Identification honesty.** 4PL parameters $(a_j, b_j, c_j, d_j)$ are not estimable from a handful of evaluated systems; estimation requires the calibration sample of evaluated systems (and/or repeated seeded runs per system under the repetition policy of [§62.1](#621-minimum-repetition--execution-policy)) declared at a calibration release, and every published parameter carries its standard error. The screening thresholds of [§63.3](#633-item-discrimination-screening-rules) apply to *estimated* parameters at re-calibration time; no item parameter is published before the minimum defensible calibration sample exists (maturity M3, [§60](#60-six-benchmark-empirical-maturity-levels-m_0-dots-m_5)). Fitting the model is computationally feasible on small samples; having estimates is not evidence that they are identified — AIST does not conflate the two.
...
class AISTMasterRunner:
    ...
    def record_evaluation(
        ...
        """
        # Deterministic order-preserving dedup: a repeated gate identifier
        # within one task is ONE gate event; severity accounting counts each
        # (task, gate) pair exactly once (Section 16.2).
        gates = list(dict.fromkeys(gate_violations or []))
        ...
        for existing in self.records:
            if existing.task_id == task_id and existing.trial_index == trial_index:
                raise ValueError(
                    f"duplicate (task_id, trial_index) = ({task_id!r}, {trial_index}); "
                    "repeated executions of a task must use a distinct trial_index "
                    "(Section 62.1 repetition policy)"
                )
        t = telemetry
        if (
            t.cost_usd < 0.0 or t.latency_sec < 0.0 or t.compute_core_seconds < 0.0
            or t.peak_memory_mb < 0.0 or t.input_tokens < 0 or t.output_tokens < 0
            or t.tool_calls < 0 or t.failed_tool_calls < 0 or t.redundant_tool_calls < 0
            or t.retries < 0 or t.successful_state_transitions < 0
        ):
            raise ValueError(
                "telemetry quantities must be non-negative (Section 6.1); "
                f"malformed telemetry rejected for task {task_id!r}"
            )
    ...
    def record_robustness_trial(self, family: str, base: float, perturbed: float, recovered: float) -> None:
        ...
        is never coerced to 0.0. Inputs must be finite and non-negative —
        ItemScore means are bounded on [0, 1] (Section 13).
        """
        for name, value in (("clean", base), ("perturbed", perturbed), ("recovered", recovered)):
            if not math.isfinite(value) or value < 0.0:
                raise ValueError(
                    f"robustness trial value {name}={value!r} must be finite and >= 0 "
                    "(ItemScore means are bounded on [0, 1]; Section 13)"
                )
    ...
    def record_cascade_trial(self, base: float, cascade: float) -> None:
        ...
        the minimum of ordinary per-family retentions. Inputs must be finite
        and non-negative (ItemScore means on [0, 1])."""
        for name, value in (("clean", base), ("cascade", cascade)):
            if not math.isfinite(value) or value < 0.0:
                raise ValueError(
                    f"cascade trial value {name}={value!r} must be finite and >= 0 "
                    "(ItemScore means are bounded on [0, 1]; Section 13)"
                )
    ...
    def compile_scorecard(self) -> Dict[str, Any]:
        ...
        for battery, items in strata.items():
            ...
            battery_summaries[battery] = {
                ...
                # Unevaluated batteries (sample_size == 0) report N/A (None)
                # for every statistic — an empty battery is never zero-filled
                # as a measurement (Sections 14.2 missing-data rule, 59.2.1
                # PARTIAL effect: diagnostics only, never zeros).
                "mean_raw_score": (round(statistics.fmean(raw_scores), 4) if raw_scores else None),
                "mean_op_score": (round(statistics.fmean(op_scores), 4) if op_scores else None),
                "ci95_op_score": ((round(ci_low, 4), round(ci_high, 4)) if items else None),
                "success_rate": (round(statistics.fmean(successes), 4) if successes else None),
                "wilson_ci95_success": ((round(wilson_low, 4), round(wilson_high, 4)) if items else None),
                "mean_latency_sec": (round(statistics.fmean([r.telemetry.latency_sec for r in items]), 2) if items else None),
                "mean_cost_usd": (round(statistics.fmean([r.telemetry.cost_usd for r in items]), 4) if items else None),
                "mean_vud": (round(statistics.fmean([r.vud_score for r in items]), 2) if items else None),
    ...
    def export_all_artifacts(self) -> None:
        ...
        #    status — never dropped, never zero-filled). An empty campaign
        #    exports a header-only CSV: deterministic schema evidence, never
        #    a missing artifact.
        with csv_path.open("w", newline="", encoding="utf-8") as f:
            ...
            for r in self.records:
                writer.writerow([
                    ...
                    r.vud_score,
                    # Binary acceptance is undefined for INVALID records (the
                    # score is N/A; Section 59.3): export N/A, never a
                    # False-as-agent-failure value.
                    ("N/A" if r.result_status == RESULT_INVALID else r.success),
                    r.result_status,
        ...
        with scorecard_path.open("w", encoding="utf-8") as f:
            ...
            for b_id, b_data in card["battery_breakdown"].items():
                if b_data["sample_size"] == 0:
                    # Unevaluated battery: N/A across the row — never 0.00,
                    # which would read as a measured zero (Section 14.2).
                    f.write(f"{b_id:<26} | {'N/A (unevaluated)':<9} | {'N/A':<9} | {'N/A':<15} | {'N/A':<8}\n")
                    continue
        ...
        #    Line 1 is the MANIFEST_HEADER carrying the run metadata required
        #    by the Section 53 schema; every subsequent line is one harvested
        #    record envelope. The writer never activates outside Zones 1/2
        #    (the hermetic firewall), and harvested payloads are buffered
        #    only from valid, gate-clean records (Section 49.1).
        is_harvesting_permitted = self.evaluation_zone in ["ZONE_1_PUBLIC_DEV", "ZONE_2_SYNTHETIC_POOL"]
        if is_harvesting_permitted:
            with manifest_path.open("w", encoding="utf-8") as f:
                f.write(json.dumps({
                    ...
                    "run_id": self.run_id,
                    "timestamp_utc": card["timestamp_utc"],
                    "harvesting_zone": self.evaluation_zone,
                    "evaluated_model_id": self.identities["model_identity"],
                    "scaffold_identity": self.identities["scaffold_identity"],
                    "record_counts": {
                        "PRM_STEP": len(self.prm_manifest_entries),
                        "DPO_PAIR": len(self.dpo_manifest_entries),
                        "STAR_TRACE": len(self.star_manifest_entries),
                    },
                }) + "\n")
...
2. **Canary Breach:** Model weights emit private holdout canary strings — *contamination evidence* in the classification of [§64.2](#642-contamination-adjusted-scoring-metrics) — and the matched contamination delta is reproduced above the catastrophic threshold ($\Delta_{\text{matched}} > 0.20$), so the investigation pipeline of [§64.2](#642-contamination-adjusted-scoring-metrics) classifies the result as confirmed contamination and revocation applies. Canary emission alone triggers quarantine and investigation; it is never, by itself, sufficient for this trigger.
